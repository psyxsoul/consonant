...
export default function Dashboard() {
    ...
    const colorMap = { red: 'red', amber: 'amber', green: 'green', cyan: 'cyan', violet: 'violet' }
    ...
    return (
        <div>
            ...
            <div className="grid-4" style={{ marginBottom: 'var(--space-6)' }}>
                <StatCard icon="📦" iconBg="var(--accent-cyan-dim)" value={stats?.total_assets?.toLocaleString() || '—'} label="Data Assets Tracked" trend="12%" trendDir="up" />
                ...
                <div className="dash-panel">
                    ...
                    {activities.map((a, i) => (
                        <div className="activity-item" key={a.id || i}>
                            <span className={`activity-dot ${colorMap[a.color] || 'cyan'}`} />
                            <div>
                    ...
                    {activities.length === 0 && <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', padding: 'var(--space-4)' }}>No recent activity</p>}
                </div>
                    ...
                    <div className="dash-panel">
                        ...
                        <div style={{ textAlign: 'center', padding: 'var(--space-4) 0' }}>
                            <div style={{ fontSize: '3rem', fontWeight: 800, background: 'var(--gradient-accent)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', lineHeight: 1 }}>
                                {stats?.consent_coverage || 94.2}%
                            </div>
                            <p style={{ fontSize: '0.85rem', marginTop: 'var(--space-2)' }}>Overall compliance across 12 control areas</p>
                        </div>
                        <div className="progress-bar" style={{ marginTop: 'var(--space-3)' }}>
                            <div className="progress-bar-fill" style={{ width: `${stats?.consent_coverage || 94.2}%` }} />
                        </div>
                    </div>
                </div>
            </div>
